...
import { createSchemaPrompt } from './schema-generator';
...
export async function promptWithStructure<T>(
  ...
  console.log('[structured-prompt] schema: ', schema);
  let jsonSchema: SchemaDefinition;
  if (schema instanceof ZodType) {
    jsonSchema = z.toJSONSchema(schema) as SchemaDefinition;
  } else {
    jsonSchema = schema;
  }
  console.log('[structured-prompt] jsonSchema: ', jsonSchema);
  const jsonSchemaString = JSON.stringify(jsonSchema);

  // Create schema-aware prompt
  const schemaInstructions = createSchemaPrompt(jsonSchemaString);
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      const response = await session.prompt(fullPrompt, {
        signal, responseConstraint: jsonSchema,
      });
...
export async function promptWithStructureStreaming<T>(
  ...
  const jsonSchemaString = JSON.stringify(z.toJSONSchema(schema as ZodType<T>));
  const schemaInstructions = createSchemaPrompt(jsonSchemaString);
  const fullPrompt = `${schemaInstructions}\n\nUser Request: ${prompt}`;
