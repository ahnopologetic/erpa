...
export const SectionHighlight: React.FC<SectionHighlightProps> = ({
    ...
    useEffect(() => {
        ...
        if (sections[currentSectionIndex]) {
            const element = document.querySelector(sections[currentSectionIndex].cssSelector) as HTMLElement
            if (element) {
    ...
    }, [currentSectionIndex, sections, onNavigateToSection])
    ...
    return (
        <div className="fixed right-4 top-1/2 transform -translate-y-1/2 z-[9999] flex flex-col gap-2">
            {/* Up arrow */}
            <button
                onClick={navigateUp}
                className="w-10 h-10 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-full shadow-lg hover:bg-white hover:shadow-xl transition-all duration-200 flex items-center justify-center group"
                title={`Previous section (${currentSectionIndex > 0 ? sections[currentSectionIndex - 1]?.title : sections[sections.length - 1]?.title})`}
